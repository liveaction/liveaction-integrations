...
# create_snmp_traps_otel_view.sh
#
# Creates the following view inside ClickHouse via the running container:
#
#   CREATE VIEW IF NOT EXISTS liveassist.SNMP_Traps_OpenTelemetry_Log_Data
#     AS SELECT * FROM default.otel_logs WHERE ServiceName = 'snmptrapreceiver';
#
# This variant uses:
...
readonly VIEW_SQL="CREATE VIEW IF NOT EXISTS liveassist.SNMP_Traps_OpenTelemetry_Log_Data AS SELECT * FROM default.otel_logs WHERE ServiceName = 'snmptrapreceiver';"

# Execute inside the container
docker exec clickhouse-server clickhouse-client --query \"$VIEW_SQL\"

echo \"OK: Ensured view liveassist.SNMP_Traps_OpenTelemetry_Log_Data exists.\"
