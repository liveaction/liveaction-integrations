...
#
#   CREATE VIEW IF NOT EXISTS liveassist.Portnox_OpenTelemetry_Log_Data
#     AS SELECT * FROM default.otel_logs WHERE ServiceName = 'portnox';
#
# This variant uses:
...
readonly VIEW_SQL="CREATE VIEW IF NOT EXISTS liveassist.Portnox_OpenTelemetry_Log_Data AS SELECT * FROM default.otel_logs WHERE ServiceName = 'portnox';"

# Execute inside the container
docker exec clickhouse-server clickhouse-client --query \"$VIEW_SQL\"

echo \"OK: Ensured view liveassist.Portnox_OpenTelemetry_Log_Data exists.\"
