...
def process_data(record):
    ...
    enabled = True
    if enabled:
        for network, [name, threshold] in B2B.items():
            ...
            if ipaddress.IPv4Address(destination_ip) in ipaddress.IPv4Network(network):
...
def threading(thread):
    with concurrent.futures.ThreadPoolExecutor() as executor:
        [executor.submit(process_data, record) for record in thread]
