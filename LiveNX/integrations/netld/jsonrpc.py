...
class JsonRpcProxy(object):
    ...
    def fromHost(cls, host, username, password):
        proxy = cls(f"https://{host}/jsonrpc", username, password)
        proxy._host = host
    ...
    def call(self, method, *args, **kwargs):
        ...
        encoded = encode(postdata).encode('utf-8')
        local_logger.info(encoded)
        try:
            with self._opener.open(url, encoded) as response:
                respdata = response.read()
        except urllib.error.URLError as ex:
            local_logger.info('Connection error: ' + str(ex))
            sys.exit(-1)
...
def dict_encode(obj):
    items = getattr(obj, 'items', obj.items)
    return {encode_(k): encode_(v) for k, v in items()}

def list_encode(obj):
    return [encode_(i) for i in obj]
